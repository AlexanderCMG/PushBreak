...
MOTOR_DOWN_DELAY = 1 * 60    # seconds - delay before sending motor down command after activity (1 minute)
...
class ActivityMonitor(DefaultDelegate):
    def __init__(self, serial_connection):
        ...
        # Motor down control
        self.motor_down_scheduled = False
        self.motor_down_time = None
        ...
        print(f"Motor down delay: {MOTOR_DOWN_DELAY/60:.1f} minutes")
        print("=" * 35)
    ...
    def handleNotification(self, cHandle, data):
        ...
        # Check for motor down command
        self._check_motor_down(current_time)
    ...
    def _handle_activity_detected(self, current_time):
        ...
        # Schedule motor down command
        self._schedule_motor_down(current_time)

    def _schedule_motor_down(self, current_time):
        """Schedule motor down command after activity is detected"""
        self.motor_down_scheduled = True
        self.motor_down_time = current_time + MOTOR_DOWN_DELAY
        print(f"Motor down scheduled for {datetime.fromtimestamp(self.motor_down_time).strftime('%H:%M:%S')}")

    def _check_motor_down(self, current_time):
        """Check if it's time to send motor down command"""
        if self.motor_down_scheduled and current_time >= self.motor_down_time:
            self._send_motor_down()

    def _send_motor_down(self):
        """Send motor down command"""
        self.ser.write(b'2')  # Send motor DOWN command
        self.motor_down_scheduled = False
        self.motor_down_time = None
        print(f"MOTOR DOWN command sent at {datetime.now().strftime('%H:%M:%S')}")
    ...
    def _print_status(self, current_steps, is_sitting):
        ...
        # Add motor down status
        down_status = ""
        if self.motor_down_scheduled:
            remaining_down = (self.motor_down_time - current_time) / 60
            down_status = f" | Down in: {remaining_down:.1f}min"
        
        print(f"[{status_time}] Steps: {current_steps:4d} | {status} | Motor: {motor_status}{down_status}")
